let handler = async (m, { conn, args }) => {
    ...
    try {
        ...
        // Panggil API NekoLabs tanpa apikey
        let response = await fetch(`https://api.nekolabs.web.id/downloader/mediafire?url=${encodeURIComponent(url)}`);
        ...
        // Opsional: batasi ukuran file agar tidak melebihi batas WhatsApp
        if (filesize.includes("GB") || parseFloat(filesize) > 100)
            return m.reply("⚠️ *Ukuran file terlalu besar untuk dikirim lewat WhatsApp!*");
